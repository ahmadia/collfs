/*
   IBM's glibc for Blue Gene is version 2.4 which includes a 
   somewhat busted fmemopen, we can't do with that and backport
   one from 2.13 along with changes to let it compile outside
   of glibc
*/
...
/*
 * myfmemopen() - "my" version of a string stream
...
FILE *
myfmemopen (void *buf, size_t len, const char *mode)
{

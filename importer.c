...
static char* sys_files[] = {
	...
	MPI_Bcast(&mtime,sizeof(time_t),MPI_BYTE,0,MPI_COMM_WORLD);
        ...
        if (cpathname != NULL &&
            ...
            if (co == NULL)
                return NULL;
            if (Py_VerboseFlag)
                PySys_WriteStderr("mpiimport %s # from %s\n",
                                  name, pathname);
            if (cpathname) {
		    ...
		    {
                    /* 
                     this is an activity that only write it does not modify
                     code; write_compiled_module returns static void, so
                     this construct ought to be totally safe.
                     */
                      if (rank == 0) write_compiled_module(co, cpathname, &st);
            	    }
		}
        }
    ...
    load_package(char *name, char *pathname)
    {
        ...
        PyObject *path = NULL;
        int err;
        char * buf; /* Changing this to calloc from buf[MAXPATHLEN+1] */
        FILE *fp = NULL;
        struct filedescr *fdp;
        
        
        buf=calloc(MAXPATHLEN+1,sizeof(char)); /* this eliminated corruption elsewhere */
        ...
        if (m == NULL)
            return NULL;
        if (Py_VerboseFlag)
            PySys_WriteStderr("mpiimport %s # directory %s\n",
                              name, pathname);
        d = PyModule_GetDict(m);
                ...
                size_t buflen, FILE **p_fp, PyObject **p_loader)
    {
...
#ifdef HAVE_STAT
            if (mpistat(buf, &statbuf) == 0 &&         /* it exists */   /* WRS: I think this is where the other ranks get confused. */
         ...
         *
         * We've already done a successful stat() or mpifopen() on buf, so know that
...
#include "oslib/osfscontrol.h"
        ...
        init_builtin(char *name)
        {
            ...
            for (p = PyImport_Inittab; p->name != NULL; p++) {
                if (strcmp(name, p->name) == 0) {
                    ...
                    if (Py_VerboseFlag)
                        PySys_WriteStderr("mpiimport %s # builtin\n", name);
                    (*p->initfunc)();
                    if (PyErr_Occurred())
                        return -1;
        ...
        PyImport_ImportFrozenModule(char *name)
        {
            ...
            if (ispackage)
                size = -size;
            if (Py_VerboseFlag)
                PySys_WriteStderr("mpiimport %s # frozen%s\n",
        ...
        PyImport_ReloadModule(PyObject *m)
        {
            ...
            if (subname == NULL)
                ...
                if (parentname == NULL) {
                    mpiimporter_modules_reloading_clear();
                    return NULL;
                }
        ...
        {
            extern int mpifclose(FILE *);
            PyObject *fob, *ret;
            struct filedescr *fdp;
            char * pathname;
            FILE *fp = NULL;
            
	    pathname=calloc(MAXPATHLEN+1,sizeof(char));

            pathname[0] = '\0';
            if (path == Py_None)
                path = NULL;
            fdp = find_module(NULL, name, path, pathname, MAXPATHLEN+1, &fp, NULL); //keep getting stuck here with a wrong answer!
            if (fdp == NULL)
                return NULL;
            if (fp != NULL) {
                fob = PyFile_FromFile(fp, pathname, fdp->mode, mpifclose);
                if (fob == NULL) {
                    mpifclose(fp);
                    return NULL;
                }
        ...
        static PyMethodDef mpiimporter_methods[] = {
            {"reload",           mpiimporter_reload,       METH_O,       doc_reload},
            {"find_module",      mpiimporter_find_module,  METH_VARARGS, doc_find_module},
            {"get_magic",        mpiimporter_get_magic,    METH_NOARGS,  doc_get_magic},
            {"get_suffixes", mpiimporter_get_suffixes, METH_NOARGS,  doc_get_suffixes},
            {"load_module",      mpiimporter_load_module,  METH_VARARGS, doc_load_module},
            {"new_module",       mpiimporter_new_module,   METH_VARARGS, doc_new_module},
            {"lock_held",        mpiimporter_lock_held,    METH_NOARGS,  doc_lock_held},
            {"acquire_lock", mpiimporter_acquire_lock, METH_NOARGS,  doc_acquire_lock},
            {"release_lock", mpiimporter_release_lock, METH_NOARGS,  doc_release_lock},
            /* The rest are obsolete */
            {"get_frozen_object",       mpiimporter_get_frozen_object,  METH_VARARGS},
            {"init_builtin",            mpiimporter_init_builtin,       METH_VARARGS},
            {"init_frozen",             mpiimporter_init_frozen,        METH_VARARGS},
            {"is_builtin",              mpiimporter_is_builtin,         METH_VARARGS},
            {"is_frozen",               mpiimporter_is_frozen,          METH_VARARGS},
            {"load_compiled",           mpiimporter_load_compiled,      METH_VARARGS},
#ifdef HAVE_DYNAMIC_LOADING
            {"load_dynamic",            mpiimporter_load_dynamic,       METH_VARARGS},
#endif
            {"load_package",            mpiimporter_load_package,       METH_VARARGS},
            {"load_source",             mpiimporter_load_source,        METH_VARARGS},
            {NULL,                      NULL}           /* sentinel */
        };
        ...
        initmpiimporter(void)
        {
            ...
            m = Py_InitModule4("mpiimporter", mpiimporter_methods, doc_mpiimporter,
